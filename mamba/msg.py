...
def err(content: str, colour: bool):
    if colour:
        sys.stderr.writelines(
            Fore.RED + Style.BRIGHT + "[!]" + Style.RESET_ALL + f" {content}\n"
        )
    else:
...
def warn(content: str, colour: bool):
    if colour:
        sys.stderr.writelines(
            Fore.YELLOW + Style.BRIGHT + "[?]" + Style.RESET_ALL + f" {content}\n"
        )
    else:
...
def warn_confirmed(content: str, colour: bool) -> bool:
    if colour:
        ...
        user_input = input(
            f"are you sure? [{Fore.GREEN}y{Style.RESET_ALL}/{Fore.RED}N{Style.RESET_ALL}]: "
        )
    else:
...
def prompt(content: str, colour: bool) -> bool:
    if colour:
        print(
            Fore.BLUE + Style.BRIGHT + "[*]" + Style.RESET_ALL + f" {content}", end=" "
        )
        user_input = input(
